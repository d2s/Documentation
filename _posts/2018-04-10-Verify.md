...
Below is sample of verify for **snapshot** synchronizing task. The output from rsync informs about a catalog `..snapshots/GitHub/72` is created. This is because the snapshot number 72 is no yet created in backup storage. The number is next snapshot in sequence and will by RsyncOSX automatically be created in next synchronizing.
