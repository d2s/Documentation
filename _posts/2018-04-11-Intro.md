...
(3) `⌘B` - shortcut for automatic executing backups, the command automatically checks **all tasks** for files to be synchronized or deleted and automatically executes tasks with files to be synchronized

(4) `⌘R` - shortcut for immediate execute **one selected** task
