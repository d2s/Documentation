...
(2) as above, but you might add a [parameters](/Parameters) to rsync to save changed and deleted files in a separate backup catalog, utilize `--exclude` parameters to rsync to exclude files from being transferred
...
(7) `⌘B` - shortcut for executing automatic backups
